...
export const DataManager = {
    ...
    getLowStockItems: async () => {
        ...
        try {
            // Logic: 
            // 1. Get all products with min_stock_level > 0
            // 2. Count current inventory for each (status = 'in_stock')
            // 3. Filter where count < min_stock_level

            // Fetch products
            const { data: products, error: prodError } = await supabase
                .from('products')
                .select('id, uuid, name, code, image_url, min_stock_level, stock') // 'stock' column in products table is often legacy simple count
                .gt('min_stock_level', 0)

            if (prodError) throw prodError

            // Fetch inventory counts (aggregated)
            // Note: If dataset is huge, this is inefficient. 
            // Better to have a database view or RPC. 
            // For now, we'll do a client-side aggregation because product count is likely < 1000.
            ...
            const lowStock = products.map(p => {
                const current = stockCounts[p.uuid] || stockCounts[p.id] || 0
                return {
                    ...p,
                    current_stock: current,
                    reorder_qty: Math.max(0, p.min_stock_level - current)
                }
    ...
    getProducts: async () => {
        ...
        try {
            ...
            // --- Aggregation Logic for Total Purchased, Total Sold, Total Lost ---
            let purchaseCounts = {}
            let salesCounts = {}
            let lostCounts = {}
            ...
            try {
                    ...
                    salesOrders.forEach(order => {
                        if (order.items && Array.isArray(order.items)) {
                            order.items.forEach(item => {
                                ...
                                // Try to match by product_id (UUID) first, then id (Code/Legacy)
                                if (item.product_id) {
                                    salesCounts[item.product_id] = (salesCounts[item.product_id] || 0) + qty
                                } else if (item.id) {
                                    // Note: item.id might be a code like "AA001". 
                                    // We'll map this later or hope the product ID map works.
                                    // For now, store it as is.
                                    salesCounts[item.id] = (salesCounts[item.id] || 0) + qty
                                }
                            })
                        }
                    })
                ...
                // 3. Fetch Lost Items (Inventory Items with status = 'lost')
                // Note: This is an aggregation of individual items.
            ...
            return data.map(p => {
                ...
                // Avoid double counting if UUID and Code are same (unlikely) or if legacy logic mixed them
                // We'll take the max or sum? Usually they are distinct keys in the map.
                // If data is clean, match by UUID is best. Fallback to Code.
                ...
                return {
                    ...
                    total_purchased: purchaseCounts[pid] || 0, // Only map by UUID for new system, or strictly ID
    ...
    getPurchaseOrders: async () => {
        try {
            const { data, error } = await supabase
                ...
                .select(`
                    *,
                    supplier:suppliers (name) -- Optional if you have suppliers table, otherwise use supplier_name column
                `)
                .order('created_at', { ascending: false })
