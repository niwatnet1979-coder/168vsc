    ...
    getOrders: async () => {
        ...
        try {
            const { data, error } = await supabase
                ...
                items:order_items(*, product:products(name, image_url), variant:product_variants!product_variant_id(*), siteAddressRecord:customer_addresses!site_address_id(*), siteInspectorRecord:customer_contacts!site_inspector_id(*))
            `)
                .order('created_at', { ascending: false })
            ...
            return data.map(o => ({
                ...
                items: (o.items || []).map(i => ({
                    ...
                    image: i.product?.image_url || i.image || null,
    ...
    saveOrder: async (orderData) => {
        try {
            ...
            const orderPayload = {
                ...
                // Save Main Job Info to JSONB column (Minimized: Relational Enforcement)
                job_info: orderData.jobInfo ? {
                    job_type: orderData.jobInfo.job_type || orderData.jobInfo.jobType,
                    ...
                    // Fields below are excluded if linked to a Master Record (Option B: Relational Only)
                    install_location_name: undefined,
                    install_address: undefined,
                    google_map_link: undefined,
                    distance: undefined,

                    // If receiver_contact_id exists, we don't store the full inspector1 JSON object here
                    inspector1: undefined,
                    inspector2: undefined,
                    inspector2Phone: undefined
                } : {}
            }
            ...
            const itemsPayload = orderData.items.map(item => {
                ...
                return {
                    ...
                    // JSONB fields (Minimized: Relational Only - Option B/Phase 4)
                    variation_data: {
                        // We no longer store snapshotted attributes like color/dimensions
                        // or the full selectedVariant object.
                        // They are re-hydrated at runtime from the Master Variant join.

                        // Essential IDs for lookup (already in table columns, but keeping for back-compat if needed)
                        // selectedVariantId: variantId, 

                        // Essential/Custom Remark data (Non-Mastered)
                        remark: undefined, // item.remark, -- Stored in column
                        description: undefined, // item.description, -- Stored in column

                        // Properties that might differ by individual installation
                        light: undefined, // item.light,
                        lightColor: undefined, // item.lightColor,
                        remote: undefined, // item.remote,
                        selectedVariant: undefined,
                        selectedVariantIndex: undefined
                    },
                    sub_job_data: item.subJob ? {
                        ...item.subJob,
                        // Phase 4 & 5 & 13 Cleanup: Remove all fields that are now relational
                        jobId: undefined,
                        jobType: undefined,
                        team: undefined,
                        ...
                        installLocationName: undefined,
                        installLocationId: undefined, // Clear from JSON, stored in column
                        googleMapLink: undefined,
                        distance: undefined
                    } : {},
                    site_address_id: item.subJob?.installLocationId || null,
    ...
    `)
            ...
            const order = {
                ...
                jobInfo: data.job_info ? {
                    jobType: data.job_info.job_type || data.job_info.jobType,
                    team: data.job_info.team,
                    appointmentDate: data.job_info.appointment_date || data.job_info.appointmentDate,
                    completionDate: data.job_info.completion_date || data.job_info.completionDate,

                    // RE-HYDRATION: Prioritize JSONB (Manual entry) then Join Data (Master Record)
                    installLocationName: data.job_info.install_location_name || finalDeliveryAddress?.label,
                    installAddress: data.job_info.install_address || finalDeliveryAddress?.address,
                    googleMapLink: data.job_info.google_map_link || finalDeliveryAddress?.googleMapsLink || finalDeliveryAddress?.googleMapLink,
                    distance: data.job_info.distance || finalDeliveryAddress?.distance,
                    inspector1: data.job_info.inspector1 || finalReceiver,

                    description: data.job_info.description,
                    // Spread remaining to catch custom fields
                    ...data.job_info
                } : {},
