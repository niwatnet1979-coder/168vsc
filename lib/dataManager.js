...
export const DataManager = {
    ...
    linkServiceFeeJobs: async (serviceFeeId, jobIds = []) => {
        try {
            ...
            // Ignore duplicates via ON CONFLICT DO NOTHING (if constraint exists) OR verify before insert
            // Using maybeSingle check or just insert and catch error?
            // Safer: Insert and ignore conflicts if possible. 
            // Supabase upsert without update?
            const { error } = await supabase
    ...
    getJobById: async (id) => {
        try {
            ...
            return {
                ...
                team: job.team_id ? job.team_id : (job.team_name || job.team), // Prefer ID, fallback to name

                // Images/Products (if stored in job, otherwise need fetch from order_items? Job should have snapshot)
                productName: job.product_name,
                productImage: job.product_image_url
            }
    ...
    `)
            ...
            const { data: allJobs, error: jobsError } = await supabase
                .from('jobs')
                .select('id, order_item_id, order_id, sequence_number, job_type, status, assigned_team, appointment_date, completion_date, site_inspector_id, site_address_id, notes, team_payment_batch_id, created_at, updated_at, siteAddressRecord:site_address_id(*), siteInspectorRecord:site_inspector_id(*)')
                .eq('order_id', id)
