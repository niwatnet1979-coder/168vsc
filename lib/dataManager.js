...
export const DataManager = {
    ...
    getOrders: async () => {
        ...
        try {
            ...
            return data.map(o => ({
                ...
                items: (o.items || []).map(i => {
                    // Sorting Jobs: Recent first (Sequence DESC preferred, then created_at)
                    const itemJobs = (i.jobs || []).sort((a, b) => {
                        // Sort by sequence number desc if available, otherwise created_at
                        if (b.sequence_number !== a.sequence_number) return (b.sequence_number || 0) - (a.sequence_number || 0);
    ...
    getOrdersByCustomerId: async (customerId) => {
        ...
        try {
            const { data, error } = await supabase
                .from('orders')
                .select('*, items:order_items(*, jobs(status, created_at))')
        ...
        try {
            ...
            const { data: orders, error } = await supabase
                ...
                .select(`
                    *,
                    ...
                    items:order_items(*, jobs(*), product:products(name, image_url, description), siteAddressRecord:customer_addresses!site_address_id(*), siteInspectorRecord:customer_contacts!site_inspector_id(*))
                `)
            ...
            orders.filter(o => calculateOrderStatus(o.items) !== 'Cancelled').forEach(order => {
                ...
                orderData.items.forEach((item, itemIdx) => {
                    ...
                    const productDescription = item.product?.description || null
    ...
    getJobById: async (id) => {
        try {
            // Refactored: Use "Virtual Job" logic exclusively (from order_items)
            console.log('[getJobById] Fetching job via Virtual Job logic for ID:', id)

            // Strategy: We need to find the order containing this job.
            // Since we don't have a direct 'jobs' table index anymore, we rely on:
            // 1. If ID is "OD...-x", we know the Order ID.
            // 2. If ID is "JB...", we might need to search order_items.job_id (this requires an index on job_id).
            // For efficiency, we'll try to guess the Order ID if possible, or use a specific RPC/Query if needed.
            // BUT, strictly following the "Virtual Job" logic from getJobs, we might search all active orders? No, that's heavy.

            // BETTER APPROACH: Query order_items directly!
            // referencing: items:order_items(*, ...)
            ...
            const { data: item, error } = await supabase
                ...
                .select(`
                    *,
                    ...
                    siteAddressRecord:customer_addresses!site_address_id(*),
                    siteInspectorRecord:customer_contacts!site_inspector_id(*),
                    order:orders(
                        *,
                        customer:customers(*),
                        deliveryAddress:customer_addresses!orders_delivery_address_id_fkey(*),
                ...
                `)
                .or(`job_id.eq.${id},id.eq.${id}`) // Try matching job_id or item id
                .maybeSingle()

            if (error) throw error
            if (!item) {
                console.log('[getJobById] Job not found in order_items for:', id)
                return null
            }

            // Construct the Job Object from this item
            const order = item.order
            // Re-use logic similar to getJobs loop but for single item
            const orderData = {
                id: order.id,
                customer: order.customer_details, // fallback to jsonb if needed, but we have joins
                date: order.order_date,
                // status: calculateOrderStatus(order.items), // calculate later
                jobInfo: {
                    job_type: order.job_type,
                    team: order.assigned_team, // Legacy order column dropped? No, might still be in code but data is in items
                    appointment_date: order.appointment_date, // dropped
                    completion_date: order.completion_date, // dropped
                    notes: order.notes // dropped
                }
            }

            // Determine Is Separate
            const mainJobType = order.job_type || '-' // order.job_type exists (we kept it)
            const isSeparate = mainJobType === 'separate' || mainJobType === 'งานแยก'

            // Sub Job Data
            const subJob = item.sub_job_data || {} // Only if heavily used, but we should prefer columns

            // Construct Data
            let jobType, appointmentDate, completionDate, team, assignedTeam, inspectorObj
            let installLocationName, installAddress, googleMapLink, distance

            // USE COLUMNS DIRECTLY (Refactored Logic)
            jobType = item.job_type || 'ไม่ระบุ'
            ...
            installLocationName = item.siteAddressRecord?.label || subJob.installLocationName || ''
            installAddress = item.siteAddressRecord?.address || subJob.installAddress || ''
            googleMapLink = item.siteAddressRecord?.google_maps_link || ''
            ...
            const jobObj = {
                id: item.job_id || item.id, // Prefer job_id
                orderId: order.id,
                order_id: order.id,
                customerId: order.customer_id,
                customerName: order.customer?.name || 'ลูกค้าทั่วไป',
                ...
                notes: item.job_notes || '',
                note: item.job_notes || '',
                description: item.job_notes || '',

                order: {
                    ...order,
                    items: order.items // Shallow headers of other items
                },
                subJob: subJob // Keep providing raw subJob for compatibility if UI needs it
            }
    ...
    saveOrder: async (orderData) => {
        try {
            ...
            const { error: deleteError } = await supabase
                .from('order_items')
                .delete()
                .eq('order_id', finalOrderId) // Use validated UUID
            ...
            const itemsPayload = orderData.items.map(item => {
                ...
                return {
                    ...
                    // total: Number(item.total || 0), // Generated column
                    // status: orderData.status || 'Pending', // REMOVED: Column dropped from order_items


                    // Phase 13: Relational Metadata (Metadata Columns)
                    // Phase 13: Relational Metadata (Metadata Columns) - MOVED TO JOBS TABLE
                    ...
                    // site_address_id: item.subJob?.installLocationId || rawDelivery?.id || null, // Moved to jobs
                    // site_inspector_id: item.subJob?.inspector1?.id || null // Moved to jobs
                }
            })
            ...
            if (itemsPayload.length > 0) {
                ...
                // 3. Create Jobs (1:N Refactor)
                // Filter items that actually have job data? 
                // Creating a job for EVERY item is safer for now as per requirement "1 item can have multiple jobs".
                // We create the FIRST job here.

                // 3. Create Jobs (1:N Refactor)
                // We map insertedItems back to original orderData.items to get the full job list
                const allJobsPayload = []

                insertedItems.forEach((newItem, idx) => {
                    const originalItem = orderData.items[idx]

                    // Priority: Use 'jobs' array if available (from new UI). 
                    // Fallback: Use 'subJob' (old UI or default).
                    // FIX: 'subJob' in React State contains the LATEST EDITS from Order Form.
                    // 'jobs' array in React State contains the LOADED data.
                    // We must merge 'subJob' into 'jobs' to ensure edits are saved.

                    let itemJobs = [...(originalItem.jobs || [])]

                    if (originalItem.subJob) {
                        const sub = originalItem.subJob
                        // Try to matching job in itemJobs
                        const matchIdx = itemJobs.findIndex(j =>
                            (j.id && sub.jobId && j.id === sub.jobId) ||
                            (j.jobId && sub.jobId && j.jobId === sub.jobId)
                        )

                        if (matchIdx !== -1) {
                            // Update existing job with new data from subJob
                            itemJobs[matchIdx] = { ...itemJobs[matchIdx], ...sub }
                        } else {
                            // If not found or itemJobs empty, add/use subJob
                            if (itemJobs.length === 0) {
                                itemJobs = [sub]
                            } else {
                                // If jobs exist but ID mismatch, it implies a new job or id change?
                                // Safest to add it as a new active job
                                itemJobs.push(sub)
                            }
                        }
                    } else if (itemJobs.length === 0) {
                        // Default if absolutely no data
                        itemJobs = [{
                    ...
                    itemJobs.forEach((job, jobIdx) => {
                        allJobsPayload.push({
                            ...
                            sequence_number: job.sequence_number || (jobIdx + 1), // Trust UI sequence or auto-increment

                            // Map CamelCase (App) -> Snake_Case (DB)
                            // Note: 'job' object might come from subJob (camelCase) or existing jobs logic
                ...
                if (allJobsPayload.length > 0) {
                    const { error: jobsError } = await supabase
                        .from('jobs')
                        .insert(allJobsPayload)

                    if (jobsError) {
                        console.error('[saveOrder] Error creating jobs:', jobsError)
                        throw jobsError
                    }
    ...
    `)
            ...
            const order = {
                ...
                jobInfo: (() => {
                    ...
                    const mainJob = (firstItem?.jobs || []).sort((a, b) => (b.sequence_number || 0) - (a.sequence_number || 0))[0] || {};
                ...
                items: (data.items || []).map(item => {
                    ...
                    // Sort Jobs (Recent First) and Map to CamelCase
                    const mappedJobs = (item.jobs || []).sort((a, b) => (b.sequence_number || 0) - (a.sequence_number || 0)).map(j => {
                    ...
                    console.log('[DEBUG] mappedJobs:', JSON.stringify(mappedJobs, null, 2))
                    const latestJob = mappedJobs[0] || {};
                    ...
                    return {
                        ...
                        subJob: {
                            ...item.sub_job_data,
                            jobId: latestJob.jobId || item.sub_job_data?.jobId,
                            jobType: latestJob.jobType || item.sub_job_data?.jobType,
                            appointmentDate: latestJob.appointmentDate || item.sub_job_data?.appointmentDate,
                            completionDate: latestJob.completionDate || item.sub_job_data?.completionDate,
                            team: latestJob.team || item.sub_job_data?.team,
                            description: latestJob.description || item.sub_job_data?.description,

                            installLocationId: latestJob.installLocationId,
                            installLocationName: latestJob.installLocationName || item.sub_job_data?.installLocationName,
                            installAddress: latestJob.installAddress || item.sub_job_data?.installAddress,
                            googleMapLink: latestJob.googleMapLink || item.sub_job_data?.googleMapLink,
                            distance: latestJob.distance || item.sub_job_data?.distance,

                            inspector1: latestJob.inspector1 || item.sub_job_data?.inspector1
                        }
                    }
                }),
