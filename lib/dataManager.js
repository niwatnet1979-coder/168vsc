    ...
    saveCustomer: async (customerData) => {
        try {
            const dbPayload = {
                ...
                // Valid DB columns are contact1, contact2 (JSONB)
                // We map the new 'contacts' array to these legacy columns to ensure data is saved
                // without needing a DB migration right now.
                contact1: (customerData.contacts && customerData.contacts[0]) ? customerData.contacts[0] : customerData.contact1,
                contact2: (customerData.contacts && customerData.contacts[1]) ? customerData.contacts[1] : customerData.contact2,
                // contacts: customerData.contacts, // REMOVE: Column does not exist in DB yet
