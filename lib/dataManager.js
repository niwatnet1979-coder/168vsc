...
export const DataManager = {
            ...
            const { data: orders, error } = await supabase
                ...
                .select(`
                    *,
                    ...
                    items:order_items(*, product:products(name, image_url, description), siteAddressRecord:customer_addresses!site_address_id(*), siteInspectorRecord:customer_contacts!site_inspector_id(*))
                `)
    ...
    saveOrder: async (orderData) => {
        try {
            ...
            const orderPayload = {
                ...
                // FK references
                purchaser_contact_id: orderData.purchaserContact?.id || null,
            ...
            if (itemsPayload.length > 0) {
                ...
                insertedItems.forEach((newItem, idx) => {
                    ...
                    if (itemJobs.length === 0 && originalItem.subJob) {
                        // Backward compatibility: Convert subJob to a job entry
                        itemJobs = [originalItem.subJob]
                    }

                    // If still empty, ensure at least one default job? 
                    // Or maybe strict 1:N implies we allow 0 jobs? 
                    // Current generic logic: default to 1 'installation' job if nothing provided?
                    ...
                    itemJobs.forEach((job, jobIdx) => {
                        allJobsPayload.push({
                            ...
                            // Location / Inspector
                            site_address_id: job.installLocationId || job.site_address_id || null,
                            site_inspector_id: job.inspector1?.id || job.site_inspector_id || null,
    ...
    getOrderById: async (id) => {
        ...
        items:order_items(*, jobs(*, siteAddressRecord:site_address_id(*), siteInspectorRecord:site_inspector_id(*)), product:products(name, description), variant:product_variants!product_variant_id(*))
    `)
                .eq('id', id)
                .single()
