    ...
    saveEmployee: async (empData) => {
        try {
            const dbPayload = {
                ...
                start_date: empData.startDate || null,
                end_date: empData.endDate || null,
                work_type: empData.workType,
                pay_type: empData.payType,
                pay_rate: empData.payRate,
                incentive_rate: empData.incentiveRate,
                citizen_id: empData.citizenId,
                birth_date: empData.birthDay || null,
    ...
    getOrderById: async (id) => {
        ...
        getJobs: async () => {
            ...
            saveCustomer: async (customerData) => {
                try {
                    const dbPayload = {
                        id: customerData.id, // If present, otherwise Supabase handles new ID for insert (if not passing ID) 
                        // However, for new items, if 'id' is passed, it tries to use it. 
                        // Strategy: if id is new (e.g. calculated locally), maybe better to let Supabase generate it? 
                        // But the app currently generates IDs locally. 
                        // Let's assume for now we might map mapped ID or let Supabase Auto-inc.
                        // Best practice with Supabase is usually valid IDs for updates, omit for creates.
                        // But the UI might expect an ID returned.
                        // For now, let's include ID if truthy.
                    ...
                    // Remove id if it's undefined/null to let Auto-increment work
                    if (!dbPayload.id) delete dbPayload.id;

                    const { data, error } = await supabase
                        .from('customers')
                        .upsert(dbPayload)
                        .select() // Return the saved data to get the new ID if needed
