...
export const DataManager = {
    ...
    saveOrder: async (orderData) => {
        try {
            ...
            if (itemsPayload.length > 0) {
                ...
                // 3.0 Delete existing jobs for this order (Full Replace Strategy)
                // CRITICAL: Delete old jobs before inserting new ones to prevent duplicates
                ...
                if (deleteJobsError) {
                    console.error('[saveOrder] Error deleting existing jobs:', deleteJobsError)
                    throw deleteJobsError
                }
                console.log('[saveOrder] Deleted existing jobs for order:', finalOrderId)
    ...
    `)
            ...
            const { data: allJobs, error: jobsError } = await supabase
                .from('jobs')
                .select('id, order_item_id, order_id, sequence_number, job_type, status, assigned_team, appointment_date, completion_date, site_inspector_id, site_address_id, notes, created_at, updated_at, siteAddressRecord:site_address_id(*), siteInspectorRecord:site_inspector_id(*)')
                .eq('order_id', id)
            ...
            const order = {
                ...
                jobInfo: (() => {
                    ...
                    const sortedJobs = firstItemJobs.slice().sort((a, b) => (a.sequence_number || 0) - (b.sequence_number || 0));
                ...
                items: (() => {
                    ...
                    const mappedJobs = itemJobs.sort((a, b) => (a.sequence_number || 0) - (b.sequence_number || 0)).map(j => {
