        ...
        try {
            ...
            if (Array.isArray(customerData.contacts) && customerData.contacts.length > 0) {
                ...
                if (validContacts.length > 0) {
                    ...
                    const contactsPayload = validContacts.map(contact => ({
                        id: isValidUUID(contact.id) ? contact.id : undefined, // Only send valid UUIDs
                    ...
                    // Re-implement deletion logic for contacts cleanly
                    // (We need to fetch existing contacts first to know what to delete, OR user passes existing list)
                    // Existing implementation relied on `customerData.contacts` having all current items. 
                    // If we assume `customerData.contacts` IS the full list of desired state:
                    // 1. Get all current DB contacts for this customer.
                    // 2. Delete those not in `customerData.contacts` (by UUID).

                    // Fetch current state for proper sync (Safest way)
                    ...
                    if (currentContacts) {
                        const payloadIds = contactsPayload
                            .map(c => c.id)
                            .filter(Boolean)
                        ...
                        if (idsToDelete.length > 0) {
                            await supabase.from('customer_contacts').delete().in('id', idsToDelete)
                        }
                    }
                } else {
                    // If all contacts were removed/empty, delete all existing for this customer
            ...
            if (Array.isArray(customerData.addresses) && customerData.addresses.length > 0) {
                // Address logic ...
                // Filter out addresses that seemingly have no content? 
                // Actually address is required, but we have a fallback 'ไม่ระบุ'. 
                // Let's filter if it was just a blank NEW form.
                // A blank new form usually has empty strings.
                // We should check if at least ONE field is filled?
                const validAddresses = customerData.addresses.filter(addr => {
                    // If it's existing (UUID), keep it (user might have cleared fields but wants to keep record?)
                    if (isValidUUID(addr.id)) return true
                    // If new, check if it has content
                ...
                if (validAddresses.length > 0) {
                    const addressesPayload = validAddresses.map(addr => {
                        ...
                        return {
                            id: isValidUUID(addr.id) ? addr.id : undefined,
                            customer_id: customerId,
                            label: addr.label,
                            // Direct mapping - column names now match UI
                    ...
                    // Upsert existing addresses
                    if (existingAddresses.length > 0) {
                        await supabase
                            .from('customer_addresses')
                            .upsert(existingAddresses)
                    }

                    // Sync/Delete addresses
                    ...
                    if (currentAddresses) {
                        const payloadIds = addressesPayload
                            .map(a => a.id)
                            .filter(Boolean)
                        ...
                        if (idsToDelete.length > 0) {
                            await supabase.from('customer_addresses').delete().in('id', idsToDelete)
                        }
                    }
                } else {
            ...
            if (Array.isArray(customerData.taxInvoices) && customerData.taxInvoices.length > 0) {
                // Filter out incomplete tax invoices (must have company name or tax ID)
                const validTaxInvoices = customerData.taxInvoices.filter(tax => {
                    if (isValidUUID(tax.id)) return true
                    return (tax.companyName && tax.companyName.trim() !== '') || (tax.taxId && tax.taxId.trim() !== '')
                })

                if (validTaxInvoices.length > 0) {
                    const taxInvoicesPayload = validTaxInvoices.map(tax => {
                        // Construct full address from detailed fields (since table doesn't have detailed columns)
                        const addressParts = [
                        ...
                        return {
                            id: isValidUUID(tax.id) ? tax.id : undefined,
                            customer_id: customerId,
                            company_name: tax.companyName,
                            tax_id: tax.taxId,
                            address_id: isValidUUID(tax.addressId) ? tax.addressId : null,
                    ...
                    if (currentTaxInvoices) {
                        const payloadIds = taxInvoicesPayload
                            .map(t => t.id)
                            .filter(Boolean)
