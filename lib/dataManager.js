        ...
        try {
            ...
            return data.map(c => ({
                ...
                addresses: (c.addresses || []).map(addr => ({
                    ...
                    addrNumber: addr.house_number,
                    addrMoo: addr.village_no,
                    addrVillage: addr.building,
                    addrSoi: addr.soi,
                    addrRoad: addr.road,
                    addrTambon: addr.subdistrict,
                    addrAmphoe: addr.district,
                    addrProvince: addr.province,
                    zipcode: addr.postcode,
                    googleMapsLink: addr.google_map_link,
    ...
    saveCustomer: async (customerData) => {
        try {
            ...
            if (Array.isArray(customerData.contacts) && customerData.contacts.length > 0) {
                // Use upsert instead of delete-and-insert to avoid FK constraint violations
                const contactsPayload = customerData.contacts.map(contact => ({
                    id: contact.id,  // Include ID for upsert
                ...
                // Delete contacts that were removed (not in the new list)
                const newContactIds = contactsPayload.map(c => c.id).filter(Boolean)
                if (customerData && customerData.contacts) {
                    const removedContactIds = customerData.contacts
                        .map(c => c.id)
                        .filter(id => id && !newContactIds.includes(id))
            ...
            if (Array.isArray(customerData.addresses) && customerData.addresses.length > 0) {
                ...
                const addressesPayload = customerData.addresses.map(addr => {
                    ...
                    const addressParts = [
                        ...
                        addr.addrProvince && `จังหวัด${addr.addrProvince}`,
                        addr.zipcode && addr.zipcode
                    ].filter(Boolean).join(' ')

                    return {
                        id: addr.id,  // Include ID for upsert
                        customer_id: customerId,
                        label: addr.label,
                        house_number: addr.addrNumber || addr.houseNumber,
                        village_no: addr.addrMoo || addr.villageNo,
                        building: addr.addrVillage || addr.building,
                        soi: addr.addrSoi || addr.soi,
                        road: addr.addrRoad || addr.road,
                        address: addressParts || addr.address || 'ไม่ระบุ',  // Fallback to prevent NULL
                        subdistrict: addr.addrTambon || addr.subDistrict || addr.subdistrict,
                        district: addr.addrAmphoe || addr.district,
                        province: addr.addrProvince || addr.province,
                        postcode: addr.zipcode || addr.postalCode || addr.postcode,
                        google_map_link: addr.googleMapsLink || addr.googleMapLink
                    }
                })
                ...
                if (existingAddresses.length > 0) {
                    await supabase
                        .from('customer_addresses')
                        .upsert(existingAddresses, { onConflict: 'id' })
                }

                // Delete addresses that were removed (not in the new list)
                const newAddressIds = addressesPayload.map(a => a.id).filter(Boolean)
                if (customerData && customerData.addresses) {
                    const removedAddressIds = customerData.addresses
                        .map(a => a.id)
                        .filter(id => id && !newAddressIds.includes(id))

                    if (removedAddressIds.length > 0) {
                        await supabase
                            .from('customer_addresses')
                            .delete()
                            .in('id', removedAddressIds)
                    }
                }
            }
            ...
            if (Array.isArray(customerData.taxInvoices) && customerData.taxInvoices.length > 0) {
                ...
                const taxInvoicesPayload = customerData.taxInvoices.map(tax => {
                    // Construct full address from detailed fields for backward compatibility
                    const addressParts = [
                    ...
                    return {
                        id: tax.id,  // Include ID for upsert
                        customer_id: customerId,
                        company_name: tax.companyName,
                        tax_id: tax.taxId,
                        address_id: tax.addressId || null,
                        address: addressParts || tax.address,  // Use constructed address or fallback
                ...
                if (newTaxInvoices.length > 0) {
                    await supabase
                        ...
                        .insert(newTaxInvoices)
                }

                // Update existing tax invoices
                if (existingTaxInvoices.length > 0) {
                    await supabase
                        .from('customer_tax_invoices')
                        .upsert(existingTaxInvoices, { onConflict: 'id' })
                }

                // Delete tax invoices that were removed (not in the new list)
                const newTaxInvoiceIds = taxInvoicesPayload.map(t => t.id).filter(Boolean)
                if (customerData && customerData.taxInvoices) {
                    const removedTaxInvoiceIds = customerData.taxInvoices
                        .map(t => t.id)
                        .filter(id => id && !newTaxInvoiceIds.includes(id))

                    if (removedTaxInvoiceIds.length > 0) {
                        await supabase
                            .from('customer_tax_invoices')
                            .delete()
                            .in('id', removedTaxInvoiceIds)
                    }
                }
            }
