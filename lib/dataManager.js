    ...
    saveCustomer: async (customerData) => {
        try {
            ...
            if (Array.isArray(customerData.contacts) && customerData.contacts.length > 0) {
                ...
                if (validContacts.length > 0) {
                    ...
                    if (newContacts.length > 0) {
                        await supabase.from('customer_contacts').insert(newContacts)
                    }
                    if (existingContacts.length > 0) {
                        await supabase.from('customer_contacts').upsert(existingContacts)
                    }
                    ...
                    if (currentContacts) {
                        const payloadIds = existingContacts.map(c => c.id) // Only track existing IDs
            ...
            if (Array.isArray(customerData.addresses) && customerData.addresses.length > 0) {
                ...
                if (validAddresses.length > 0) {
                    ...
                    if (newAddresses.length > 0) {
                        await supabase.from('customer_addresses').insert(newAddresses)
                    }
                    if (existingAddresses.length > 0) {
                        await supabase.from('customer_addresses').upsert(existingAddresses)
                    }
                    ...
                    if (currentAddresses) {
                        const payloadIds = existingAddresses.map(a => a.id)
            ...
            if (Array.isArray(customerData.taxInvoices) && customerData.taxInvoices.length > 0) {
                // Filter out incomplete tax invoices (must have BOTH company name AND tax ID for new records)
                const validTaxInvoices = customerData.taxInvoices.filter(tax => {
                    if (isValidUUID(tax.id)) return true
                    // For new records, must have BOTH to satisfy NOT NULL db constraints
                ...
                if (validTaxInvoices.length > 0) {
                    const taxInvoicesPayload = validTaxInvoices.map(tax => {
                        ...
                        if (isValidUUID(tax.addressId)) {
                            extraFields.address_id = tax.addressId
                        }
                    ...
                    if (newTaxInvoices.length > 0) {
                        await supabase.from('customer_tax_invoices').insert(newTaxInvoices)
                    }
                    if (existingTaxInvoices.length > 0) {
                        await supabase.from('customer_tax_invoices').upsert(existingTaxInvoices)
                    }
                    ...
                    if (currentTaxInvoices) {
                        const payloadIds = existingTaxInvoices.map(t => t.id)
