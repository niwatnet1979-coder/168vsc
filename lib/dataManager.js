...
export const DataManager = {
    ...
    getOrders: async () => {
        ...
        try {
            const { data, error } = await supabase
                .from('orders')
                // JOIN with customers AND order_items
                .select('*, customer:customers!customer_id(id, name, phone, email), items:order_items(*)')
                .order('created_at', { ascending: false })
    ...
    getJobById: async (id) => {
        try {
            ...
            return {
                ...
                installLocationName: subJobData?.installLocationName || job.install_location_name || job.installLocationName || '',
    ...
    saveProduct: async (productData) => {
        try {
            ...
            const dbPayload = {
                ...
                variants: variants, // JSONB backup
                image_url: allImages[0] || null,

                // Legacy Aliases (Synced by trigger but good to have in payload)
                code: productData.product_code || productData.id,
    ...
    saveOrder: async (orderData) => {
        try {
            ...
            const orderPayload = {
                ...
                // FK references
                selected_contact_id: orderData.purchaserContact?.id || null, // Primary (Purchaser)
                receiver_contact_id: orderData.receiverContact?.id || null, // Secondary (Receiver)

                delivery_address_id: rawDelivery?.id || null,
                tax_invoice_id: orderData.taxInvoice?.id || null,

                // JSONB Fallbacks
                delivery_address_info: (!rawDelivery?.id && rawDelivery) ? rawDelivery : null,

                selected_contact_info: (!orderData.purchaserContact?.id && orderData.purchaserContact)
                    ? orderData.purchaserContact
                    : null,

                receiver_contact_info: (!orderData.receiverContact?.id && orderData.receiverContact)
                    ? orderData.receiverContact
                    : null,

                tax_invoice_info: (!orderData.taxInvoice?.id && orderData.taxInvoice) ? orderData.taxInvoice : null,
                ...
                // Save Main Job Info to JSONB column
                job_info: orderData.jobInfo ? {
                    job_type: orderData.jobInfo.jobType,
                    team: orderData.jobInfo.team,
                    appointment_date: orderData.jobInfo.appointmentDate,
                    completion_date: orderData.jobInfo.completionDate,
                    install_location_name: orderData.jobInfo.installLocationName,
                    install_address: orderData.jobInfo.installAddress,
                    google_map_link: orderData.jobInfo.googleMapLink,
                    distance: orderData.jobInfo.distance,
                    inspector1: orderData.jobInfo.inspector1,
                    description: orderData.jobInfo.description
                } : {},
            ...
            const itemsPayload = orderData.items.map(item => {
                ...
                // If item has selectedVariant with an ID (from new getProducts), use it.
                // Fallback: If migration script ran, maybe we should try to lookup? 
                // Ideally UI sends the variant ID.
                const variantId = item.selectedVariant?.id || item.variant_id || item.variantId || null

                return {
                    order_id: orderData.id,
                    product_id: isUUID ? productIdentifier : (item.selectedVariant?.product_id || null),
                    product_variant_id: variantId,

                    // Common fields
                    quantity: Number(item.qty || item.quantity || 1),
                    unit_price: Number(item.unitPrice || item.price || 0),
                    // total: Number(item.total || 0), // Removed: Generated column (total_price)
                    discount: item.discount || 0,
                    status: orderData.status || 'Pending', // Add status to item for easier filtering in View

                    // JSONB fields for rich data (Legacy support + extra details)
                    variation_data: {
                    ...
                    // name: item.name, // Removed: Not in schema
                    // code: productIdentifier, // Removed: Not in schema
                }
            })
            ...
            const jobsPayload = orderData.items
                ...
                .map(item => {
                    ...
                    return {
                        ...
                        customer_id: orderData.customer?.id,
                        customer_name: orderData.customer?.name,
                        product_id: item.code || item.product_code || item.product?.product_code || 'UNKNOWN', // Ensure product_id is not null
    ...
    saveJob: async (jobData) => {
        try {
            ...
            const dbPayload = {
                ...
                google_map_link: jobData.googleMapLink || '',
                distance: jobData.distance || null,
                address: jobData.installAddress || jobData.address || ''
            }
    ...
    getOrderById: async (id) => {
        ...
        try {
            ...
            const finalDeliveryAddress = resolvedDeliveryAddress || data.deliveryAddress || data.delivery_address_info || defaultAddress
            ...
            } : (data.selected_contact_info || null)
            ...
            } : (data.receiver_contact_info || null)
            ...
            const order = {
                ...
                customer_name: data.customer?.name || data.customer_name || 'Unknown',
                ...
                items: (data.items || []).map(item => ({
                    ...
                    // Restore JSONB data
                    selectedVariant: item.variation_data?.selectedVariant,
                    selectedVariantIndex: item.variation_data?.selectedVariantIndex,
                    name: item.variation_data?.name,
                    code: item.variation_data?.code,
                    color: item.variation_data?.color,
                    dimensions: item.variation_data?.dimensions,
                    crystalColor: item.variation_data?.crystalColor,
                    bulbType: item.variation_data?.bulbType,
                    light: item.variation_data?.light,
                    lightColor: item.variation_data?.lightColor,
                    remote: item.variation_data?.remote,
                    remark: item.variation_data?.remark,
                    description: item.variation_data?.description,
                ...
                jobInfo: data.job_info ? {
                    jobType: data.job_info.job_type,
                    team: data.job_info.team,
                    appointmentDate: data.job_info.appointment_date,
                    completionDate: data.job_info.completion_date,
                    installLocationName: data.job_info.install_location_name,
                    installAddress: data.job_info.install_address,
                    googleMapLink: data.job_info.google_map_link,
                    distance: data.job_info.distance,
                    inspector1: data.job_info.inspector1,
