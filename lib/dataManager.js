...
export const DataManager = {
    ...
    getOrders: async () => {
        ...
        try {
            ...
            taxInvoice:customer_tax_invoices!tax_invoice_id(*),
                items:order_items(*, jobs(*), product:products(name, description), variant:product_variants!product_variant_id(*), siteAddressRecord:customer_addresses!site_address_id(*), siteInspectorRecord:customer_contacts!site_inspector_id(*))
            `)
                .order('created_at', { ascending: false })
            ...
            return data.map(o => ({
                ...
                // Job info - Derived from First Item (Status Quo for Unified Mode)
                // Note: If distinct_items mode, this only shows main/first job.
                jobType: o.job_type || o.items?.[0]?.job_type || '-',
                team: o.items?.[0]?.assigned_team || '-',
                appointmentDate: o.items?.[0]?.appointment_date || null,
                completionDate: o.items?.[0]?.completion_date || null,
                notes: o.items?.[0]?.job_notes || '',
                // Financial info
                ...
                items: (o.items || []).map(i => {
                    // Sorting Jobs: Recent first
                    const itemJobs = (i.jobs || []).sort((a, b) =>
                        new Date(b.created_at || b.appointment_date || 0) - new Date(a.created_at || a.appointment_date || 0)
                    )
                    ...
                    return {
                        ...
                        googleMapLink: (i.siteAddressRecord?.google_maps_link) || i.google_map_link || i.sub_job_data?.googleMapLink, // This needs to be in jobs table too ideally? For now keep mapped.
    ...
    saveOrder: async (orderData) => {
        try {
            ...
            if (itemsPayload.length > 0) {
                ...
                const jobsPayload = insertedItems.map((newItem, idx) => {
                    const sourceItem = itemsPayload[idx]
                    return {
                        order_item_id: newItem.id,
                        order_id: orderData.id,
                        sequence_number: 1, // Start with Job #1

                        job_type: sourceItem.job_type || 'installation',
                        status: sourceItem.status || 'รอดำเนินการ',

                        assigned_team: sourceItem.assigned_team,
                        appointment_date: sourceItem.appointment_date,
                        completion_date: sourceItem.completion_date,

                        notes: sourceItem.job_notes,

                        site_address_id: sourceItem.site_address_id,
                        site_inspector_id: sourceItem.site_inspector_id,
                ...
                if (jobsPayload.length > 0) {
                    ...
                    if (jobsError) {
                        console.error('[saveOrder] Error creating initial jobs:', jobsError)
                        throw jobsError
                    }
                    console.log('[saveOrder] Created', jobsPayload.length, 'initial jobs')
                }
            }
    ...
    getOrderById: async (id) => {
        ...
        items:order_items(*, product:products(name, description), variant:product_variants!product_variant_id(*), siteAddressRecord:customer_addresses!site_address_id(*), siteInspectorRecord:customer_contacts!site_inspector_id(*))
    `)
                .eq('id', id)
                .single()
            ...
            const order = {
                ...
                // Joined Relations (Re-hydration for Minimized JSONB)
                jobInfo: {
                    jobType: data.job_type || data.items?.[0]?.job_type || data.job_info?.jobType,
                    team: data.items?.[0]?.assigned_team || data.job_info?.team,
                    appointmentDate: data.items?.[0]?.appointment_date || data.job_info?.appointmentDate,
                    completionDate: data.items?.[0]?.completion_date || data.job_info?.completionDate,
                    description: data.items?.[0]?.job_notes || data.job_info?.description,
