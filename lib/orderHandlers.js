...
export function createOrderHandlers(formState, orderData, router, refs = {}) {
    ...
    const handleSaveOrder = async (fetchOrderData) => {
        console.log('=== handleSaveOrder CALLED ===')

        // CRITICAL FIX: Guard MUST be first, before ANY other logic
        // This prevents rapid multiple calls from showing multiple confirm dialogs
        if (refs.savingRef && refs.savingRef.current) {
            console.log('[handleSaveOrder] Already saving, skipping duplicate call')
            return
        }

        // Set guard immediately to block subsequent calls
        ...
        try {
            ...
            if (initialOrderData) {
                ...
                isDirty = !isDeepEqual(cleanForCompare(currentDataForCompare), cleanForCompare(initialOrderData))
            }
            ...
            // window.confirm() removed - now handled by ConfirmDialog in Order.jsx
