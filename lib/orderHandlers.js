...
export function createOrderHandlers(formState, orderData, router, refs = {}) {
    ...
    const handleSaveItem = (itemData, setEditingItemIndex = null) => {
        const updatedItems = [...items]
        // FIX: Check for both tempId AND database id
        const existingIndex = updatedItems.findIndex(i =>
            (itemData.tempId && i.tempId === itemData.tempId) ||
    ...
    const handleSaveOrder = async (fetchOrderData, extraPayload = {}) => {
        ...
        if (refs.savingRef && refs.savingRef.current) {
            ...
            return { success: false, message: 'กำลังบันทึกข้อมูลอยู่แล้ว' }
        }
        ...
        try {
            ...
            if (!customer?.id) {
                ...
                return { success: false, message: 'กรุณาเลือกลูกค้า' }
            }

            if (items.length === 0) {
                console.warn('No items in order')
                if (refs.savingRef) refs.savingRef.current = false
                return { success: false, message: 'กรุณาเพิ่มสินค้าอย่างน้อย 1 รายการ' }
            }
            ...
            console.log('[handleSaveOrder] Sending payload to OrdersAPI:', newOrder)
            ...
            if (result === true || result?.success) {
                ...
                return {
                    success: false,
                    message: result?.message || 'บันทึกไม่สำเร็จ (ไม่ทราบสาเหตุ)',
                    error: result?.error
                }
            }
        } catch (error) {
            ...
            return {
                success: false,
                message: 'เกิดข้อผิดพลาดในการบันทึก: ' + (error.message || 'Unknown Error'),
                error: error
            }
        } finally {
            if (refs.savingRef) refs.savingRef.current = false
