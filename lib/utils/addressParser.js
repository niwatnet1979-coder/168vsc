...
function extractAddressComponents(addressText) {
    ...
    // 1. Province
    if (!extract(/(จังหวัด|จ\.|Province)\s+([\u0E00-\u0E7Fa-zA-Z]+)/i, 'province')) {
    ...
    // 2. District
    extract(/(อำเภอ|อ\.|เขต|District|Amphoe)\s+([\u0E00-\u0E7Fa-zA-Z]+)/i, 'district');

    // 3. Subdistrict
    extract(/(ตำบล|ต\.|แขวง|Tambon|Subdistrict)\s+([\u0E00-\u0E7Fa-zA-Z]+)/i, 'subdistrict');

    // 4. Road
    extract(/(ถนน|ถ\.|Road|Rd\.)\s+([\u0E00-\u0E7Fa-zA-Z]+)/i, 'road');

    // 5. Soi/Lane
    extract(/(ซอย|ซ\.|Soi|S\.)\s+([\u0E00-\u0E7Fa-zA-Z0-9\s]+?)(?=\s|$)/i, 'lane');
