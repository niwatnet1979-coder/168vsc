/**
 * Universal Address Parser - Version 3.3
 * Skip all non-Thai text after company name
 */

const PARSER_VERSION = '3.3';
...
function findSplitPoint(text) {
    ...
    if (thaiMatch) {
        ...
        // Skip everything until we hit "หมู่" or Thai text
        // This includes: spaces, numbers, slashes, "สาขา", etc.
        const skipMatch = afterCompany.match(/^[^ก-๙]+/);
        let splitPoint = companyEnd;
        if (skipMatch) {
            splitPoint += skipMatch[0].length;
        }

        console.log('🔍 Split at:', splitPoint, '| Company:', text.substring(0, companyEnd).trim());
        console.log('🔍 Skipped:', text.substring(companyEnd, splitPoint));
        return splitPoint;
    }
