/**
 * Universal Address Parser - Version 3.1
 * Fixed regex patterns - removed lookahead
 */

const PARSER_VERSION = '3.1';
...
function findSplitPoint(text) {
    ...
    if (thaiMatch) {
        ...
        // Skip spaces and any leftover numbers/slashes
        const afterCompany = text.substring(splitPoint);
        const skipMatch = afterCompany.match(/^[\s\/\d]*/);
        if (skipMatch) {
            splitPoint += skipMatch[0].length;
        }
        console.log('🔍 Split at:', splitPoint, '| Company:', text.substring(0, splitPoint).trim());
        return splitPoint;
    }
