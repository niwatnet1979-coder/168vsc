...
function findSplitPoint(text) {
    // Thai company: "บริษัท ... จำกัด" then find where address starts
    ...
    if (thaiMatch) {
        ...
        // Find first house number AFTER company
        const numberMatch = afterCompany.match(/\d+\/\d+/);
        if (numberMatch) {
            // Split BEFORE the number (at company end), not AT the number
            const splitPoint = companyEnd;
            console.log('🔍 Split point (Thai):', splitPoint);
            console.log('  Company:', text.substring(0, splitPoint));
            console.log('  Address:', text.substring(splitPoint));
            return splitPoint;
        }
        return companyEnd;
    }
    ...
    // Fallback: split BEFORE first number
