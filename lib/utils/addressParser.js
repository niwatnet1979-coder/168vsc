...
function extractGlobalTokens(text) {
    ...
    // Zipcode (5 digits, not part of tax ID, prefer higher numbers for actual zipcodes)
    const zipcodeMatches = text.match(/(?<!\d)(\d{5})(?!\d)/g);
    if (zipcodeMatches) {
        // If multiple 5-digit numbers, prefer the one that looks like a real zipcode (> 10000)
...
function findSplitPoint(text) {
    ...
    const suffixRegex = /(บริษัท[^\d]+จำกัด|หจก[^\d]+|Company[^\d]+Ltd\.|Co\.,?\s*Ltd\.)/gi;
    let lastSuffixEnd = -1;
    let match;
