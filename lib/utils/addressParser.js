/**
 * Universal Address Parser - Version 2.1
...
function extractGlobalTokens(text) {
    ...
    // 6. Zipcode (5 digits, prefer > 10000)
 ...
 * IMPROVED: Better regex for Thai company names
 */
function findSplitPoint(text) {
    // For Thai: "บริษัท [words] จำกัด" - capture all Thai/English words between
    const thaiMatch = text.match(/บริษัท\s+([\u0E00-\u0E7Fa-zA-Z\s]+?)\s+จำกัด/i);
    if (thaiMatch) {
        const companyEnd = thaiMatch.index + thaiMatch[0].length;
        // Find first number pattern after company
 ...
 * IMPROVED: Better extraction and cleanup
 */
function extractAddressComponents(addressText) {
    ...
    // 7. House number (prioritize start of string)
...
export function parseUniversalAddress(inputText) {
    ...
    // 2. Extract global tokens (and remove!)
