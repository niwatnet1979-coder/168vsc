        ...
        for (const itemWithId of itemsWithIds) {
            ...
            for (const job of itemJobs) {
                ...
                if (!finalDistance && mapLink) {
                    try {
                        ...
                        if (!coords && typeof window !== 'undefined') {
                            ...
                            if (res.ok) {
                                ...
                                if (data.distanceNumeric) {
                                    finalDistance = data.distanceNumeric
                        ...
                        if (coords && !finalDistance) {
                            // FIX: Save as number for double precision column
                            const distStr = calculateDistance(SHOP_LAT, SHOP_LON, coords.lat, coords.lon)
                            finalDistance = parseFloat(distStr)
                ...
                const payload = {
                    ...
                    google_map_link: mapLink || null // Save map link if available
                }
