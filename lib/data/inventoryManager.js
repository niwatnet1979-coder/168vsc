 ...
 */
// receivePurchaseOrder REMOVED for v2 Smart Inbound
export const receivePurchaseOrder = async (poId) => {
    throw new Error("This function is deprecated. Please use the new Inbound Station.")
}
...
// Temp Receiving System REMOVED for v2 Smart Inbound
...
export const deletePurchaseOrder = async (id) => {
    if (!supabase) return { success: false, message: 'Non-connected client' }
    try {
        // 1. Delete PO Items
        await supabase
            .from('purchase_order_items')
        ...
        // 2. Delete Temp Receiving Items (Scanning Queue)
        // If not deleted, they will block PO deletion due to FK
        await supabase
        ...
        if (error) {
            ...
            if (error.code === '23503') {
                console.error("Delete PO FK Error:", error)
                return {
                    success: false,
                    message: "ไม่สามารถลบใบสั่งซื้อได้ เนื่องจากมีการรับสินค้าเข้าคลังแล้ว (Inventory Items) หรือมีการใช้งานในส่วนอื่น \n\nกรุณาลบรายการสินค้าในคลังที่เกี่ยวข้องกับ Lot นี้ก่อน"
                }
            }
            throw error
        }
        ...
        return {
            success: false,
            message: error.message || "เกิดข้อผิดพลาดในการลบใบสั่งซื้อ"
        }
    }
}
 ...
 * Updates status to 'shipping' and sets the shipped/expected date
 */
export const markPurchaseOrderAsShipped = async (id, shippedDate) => {
    if (!supabase) return false
    try {
        const { error } = await supabase
            .from('purchase_orders')
            .update({
                status: 'shipping',
                expected_date: shippedDate, // Use expected_date to track when it was shipped/expected
                updated_at: new Date()
            })
            .eq('id', id)
        ...
        console.error("Error marking PO as shipped:", error)
        throw error
    }
}
