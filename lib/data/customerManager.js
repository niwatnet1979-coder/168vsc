    ...
    try {
        ...
        if (customerData.contacts && Array.isArray(customerData.contacts)) {
            for (const contact of customerData.contacts) {
                ...
                await supabase.from('customer_contacts').upsert(contactPayload)
            }
        }
        ...
        if (customerData.addresses && Array.isArray(customerData.addresses)) {
            for (const addr of customerData.addresses) {
                ...
                await supabase.from('customer_addresses').upsert(addrPayload)
            }
        }

        // 4. Save tax invoices
        if (customerData.taxInvoices && Array.isArray(customerData.taxInvoices)) {
            for (const tax of customerData.taxInvoices) {
                ...
                await supabase.from('customer_tax_invoices').upsert(taxPayload)
            }
        }
