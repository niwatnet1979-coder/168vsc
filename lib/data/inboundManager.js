...
export const bindLpnToProduct = async ({
    ...
    try {
        ...
        const newItem = {
            ...
            lpn_type: 'master', // Default to master unit
        ...
        if (evidencePhotos && evidencePhotos.length > 0) {
            const photosPayload = evidencePhotos.map(photo => {
                ...
                const photoTags = typeof photo === 'string' ? ['inbound', 'binding'] : (photo.tags || ['inbound', 'binding'])
...
export const bindLpnSetToProduct = async ({
    ...
    try {
        ...
        for (let i = 0; i < totalBoxes; i++) {
            ...
            // Save Evidence (Link photos to EACH item in the set for completeness)
            if (evidencePhotos && evidencePhotos.length > 0) {
                const photosPayload = evidencePhotos.map(photo => {
                    const photoUrl = typeof photo === 'string' ? photo : photo.url
                    const photoTags = typeof photo === 'string' ? ['inbound', 'binding', 'multi-box'] : (photo.tags || ['inbound', 'binding', 'multi-box'])
