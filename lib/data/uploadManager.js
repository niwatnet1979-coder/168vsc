...
/**
 * Upload reimbursement slip
 */
export const uploadReimbursementSlip = async (file, poId) => {
    if (!supabase || !file) return null
    try {
        const timestamp = Date.now()
        const fileExt = file.name?.split('.').pop() || 'jpg'
        const filePath = `reimbursements/${poId}/${timestamp}.${fileExt}`

        const { error } = await supabase.storage
            .from('payment-slips') // Use existing bucket
            .upload(filePath, file)
        ...
        console.error('Error uploading reimbursement slip:', error)
        // Fallback to generic bucket if specific one fails (optional, but good for robustness)
        return null
    }
}
