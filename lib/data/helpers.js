...
/**
 * Helper to extract code from string format "[Code] [Name]"
 * Example: "BK ดำ" -> "BK", "ST สแตนเลส" -> "ST"
 * If no space found, returns first 2 chars or default
 */
export const parseCode = (value, fallback = 'XX') => {
    if (!value) return fallback
    const parts = value.trim().split(' ')
    if (parts.length > 0) {
        const code = parts[0]
        // If it looks like a code (all caps letters or mixed alphanumeric)
        // Usually codes are 2-3 characters
        return code
    }
    return fallback
}
