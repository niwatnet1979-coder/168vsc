...
export const DataManager = {
    ...
    getCustomers: async () => {
        ...
        try {
            ...
            return data.map(c => ({
                ...
                phone: c.phone,
                email: c.email,
                lineId: c.line || c.line_id,
                line: c.line || c.line_id, // Alias for app
                facebook: c.facebook,
    ...
    getJobById: async (id) => {
        try {
            ...
            const productMaster = orderItem?.product
            const variant = orderItem?.variant
            const productName = productMaster?.name || job.product_name || 'สินค้าไม่ระบุ'
            const productImage = variant?.image_url || ((variant?.images && variant?.images?.length > 0) ? variant.images[0] : null) || job.product_image_url || null

            // Construct full product/item object for UI (matches OrderItemModal expectation)
            const productData = orderItem ? {
                ...orderItem, // Gets id, quantity, unit_price, light, light_color, remote, remark, etc.

                // Map legacy/mismatched fields
                product_code: productMaster?.product_code || productMaster?.id,
                name: productName,
                description: productMaster?.description,
                category: productMaster?.category,
                subcategory: productMaster?.subcategory,
                material: productMaster?.material,

                // Variant info
                variant_id: variant?.id,
                color: variant?.color,
                crystalColor: variant?.crystal_color, // Map snake_case from DB to camelCase for UI
                sku: variant?.sku,
                image: productImage, // Legacy prop
                images: variant?.images || [], // For gallery

                // Dimensions
                dimensions: variant?.dimensions, // Obj
                width: variant?.size ? null : (variant?.dimensions?.width), // Fallback
                length: variant?.size ? null : (variant?.dimensions?.length),
                height: variant?.size ? null : (variant?.dimensions?.height),

                // Map Order Item specific columns to UI Props if naming differs
                qty: orderItem.quantity,
                unitPrice: orderItem.unit_price,
                lightColor: orderItem.light_color,
                bulbType: orderItem.light, // 'light' column stores bulb type

                // Helpful flags
                is_set: orderItem.is_set
            } : null
            ...
            return {
                ...
                productId: productMaster?.product_code || job.orderItem?.product_id,
    ...
    saveOrder: async (orderData) => {
        try {
            ...
            console.log('[saveOrder] Upserting order with payload:', JSON.stringify(orderPayload, null, 2))
            ...
            if (orderError) {
                ...
                console.error('[saveOrder] Error code:', orderError.code)
                console.error('[saveOrder] Error message:', orderError.message)
                console.error('[saveOrder] Error details:', orderError.details)
                throw orderError
            }

            console.log('[saveOrder] Order saved successfully with ID:', savedOrder.id)
    ...
    deleteOrder: async (id) => {
        ...
        try {
            ...
            // 1. Get job IDs to delete related team_service_fee_jobs
            ...
            console.error('Error deleting order (full):', error)
            alert(`ไม่สามารถลบได้: ${error.message} (Code: ${error.code})`)
            return false
        }
    },
                    ...
                    return (data.items || []).map(item => {
                        ...
                        const mappedJobs = itemJobs.sort((a, b) => new Date(a.created_at || 0) - new Date(b.created_at || 0)).map(j => {
                            ...
                            return {
                                ...
                                description: j.notes,
                                serviceFeeId: j.team_payment_batch_id,
